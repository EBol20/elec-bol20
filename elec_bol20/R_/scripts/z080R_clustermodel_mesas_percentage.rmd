...
```{r}
cluster_percentages = testprediction[[3]]

mes_clus = dplyr::full_join(mesas_2020, cluster_mask, by ="ID_MESA")%>%
  dplyr::select(ID_MESA, cluster, HAB)

combined_perc = dplyr::full_join(cluster_percentages, mes_clus, by = "cluster")%>%
  dplyr::select(c("ID_MESA", "cluster","HAB", contains("mean")))
  

write.csv(combined_perc, paste0(here::here(),"/../datos_1_intermedios/2020/clustermodel/clustermodel_mesas_percentage_actual.csv"), row.names = FALSE, quote = TRUE, fileEncoding = "UTF-8")
```
