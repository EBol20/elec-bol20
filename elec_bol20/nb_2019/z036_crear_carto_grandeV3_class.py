...
class CartoPlots:
    ...
    PALETTE = ebu.P_DIF
    CART_SLIDER_INIT = .5
    FILE_OUT = ebu.DIR + '/htlml_1_intermedios/2019/z036_plot_test.html'
    ...
    def plot_cart(self, data):
        bokeh.plotting.output_file(self.FILE_OUT)
        cart_init_val = self.CART_SLIDER_INIT
        data['x'] = data['LON'] * (1 - cart_init_val) + data['X'] * cart_init_val
        data['y'] = data['LAT'] * (1 - cart_init_val) + data['Y'] * cart_init_val

        # COLOR

        cm = linear_cmap('d_mas_cc', palette=self.PALETTE, low=self.C_BAR_LOW, high=self.C_BAR_HIGH)
        ...
        cart_fig.title.text = self.BAR_TITLE
